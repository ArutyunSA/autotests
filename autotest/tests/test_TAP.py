...
from autotest.tests.checks.formChecks import *
from autotest.tests.config.conftest import *
...
#==========test-cases/597 Валидация полей "Серия" и "Номер" с флажком "Бланк" в форме рецепта=================
@pytest.mark.regress_tap
def test_3_cases_597():
    case_name('Валидация полей "Серия" и "Номер" с флажком "Бланк" в форме рецепта')
...
def test_30_cases_662(criptoPro):
...
def test_32_cases_661(criptoPro):
    ...
    f.click_element(BT_cancel_recipe)
    time.sleep(3)
    if f.text(ST_recipe_grid) == 'Отменен':
        true_step('ОР: Рецепт отменен')
    else:
        false_step('Ошибка: Статус рецепта не перешел в "Отменен"')
